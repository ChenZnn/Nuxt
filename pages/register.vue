    ...
    <div class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 pt-24 pb-12">
      <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-md">
        ...
        <div class="text-center mb-10">
          ...
          <p class="mt-3 text-gray-600">Créez votre compte pour accéder à toutes les fonctionnalités.</p>
        </div>

        <!-- Formulaire -->
        <form @submit.prevent="handleRegister" class="bg-white/80 backdrop-blur-sm p-8 rounded-2xl shadow-xl space-y-6">
          <div class="space-y-2">
          ...
          <div class="space-y-2">
            ...
            <p class="text-xs text-gray-500 mt-1">Laissez vide pour utiliser un avatar par défaut</p>
          </div>
...
const email = ref('');
const password = ref('');
const avatar = ref('https://picsum.photos/800'); // URL par défaut pour l'avatar
...
const handleRegister = async () => {
  try {
    ...
    const userData = {
      ...
      avatar: avatar.value || 'https://picsum.photos/800', // Utiliser l'avatar par défaut si non fourni
    ...
    if (response.ok) {
      ...
      alert(`Erreur lors de l'inscription: ${errorData.message || 'Veuillez vérifier vos informations'}`);
    }
  } catch (error) {
